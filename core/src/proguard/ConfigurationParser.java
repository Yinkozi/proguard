...
public class ConfigurationParser
{
    ...
    throws ParseException, IOException
    {
        while (nextWord != null)
        {
            ...
            else if (ConfigurationConstants.DONT_KEEP_OPTION                                 .startsWith(nextWord)) configuration.keep                                  = parseDontKeepClassSpecificationArguments(configuration.keep, true,  false, false, null);
            else if (ConfigurationConstants.DONT_KEEP_CLASS_MEMBERS_OPTION                   .startsWith(nextWord)) configuration.keep                                  = parseDontKeepClassSpecificationArguments(configuration.keep, false, false, false, null);
            else if (ConfigurationConstants.DONT_KEEP_CLASSES_WITH_MEMBERS_OPTION            .startsWith(nextWord)) configuration.keep                                  = parseDontKeepClassSpecificationArguments(configuration.keep, false, true,  false, null);
            else if (ConfigurationConstants.DONT_KEEP_NAMES_OPTION                           .startsWith(nextWord)) configuration.keep                                  = parseDontKeepClassSpecificationArguments(configuration.keep, true,  false, true,  null);
            else if (ConfigurationConstants.DONT_KEEP_CLASS_MEMBER_NAMES_OPTION              .startsWith(nextWord)) configuration.keep                                  = parseDontKeepClassSpecificationArguments(configuration.keep, false, false, true,  null);
            else if (ConfigurationConstants.DONT_KEEP_CLASSES_WITH_MEMBER_NAMES_OPTION       .startsWith(nextWord)) configuration.keep                                  = parseDontKeepClassSpecificationArguments(configuration.keep, false, true,  true,  null);
            else if (ConfigurationConstants.PRINT_SEEDS_OPTION                               .startsWith(nextWord)) configuration.printSeeds                            = parseOptionalFile();
            else if (ConfigurationConstants.NAME_SEED_OPTION                                 .startsWith(nextWord)) configuration.nameSeed                              = parseLongArgument();
            else if (ConfigurationConstants.UTF8_NAMES_OPTION                                .startsWith(nextWord)) configuration.utf8Names                             = parseNoArgument(true);
            ...
            else if (ConfigurationConstants.PRINT_MAPPING_JSON_OPTION                         .startsWith(nextWord)) configuration.printMappingJson                     = parseOptionalFile();
    ...
    private long parseLongArgument()
    throws ParseException, IOException
    {
        try
        {
            // Read the obligatory long.
            readNextWord("long");

            long value = Long.parseLong(nextWord);
            if (value < 0)
            {
                throw new ParseException("Expecting value >= 0, instead of '" + nextWord +
                                         "' before " + reader.locationDescription());
            }
        ...
        {
            throw new ParseException("Expecting long argument instead of '" + nextWord +
                                     "' before " + reader.locationDescription());
        }
    }
    ...
    private List parseDontKeepClassSpecificationArguments(List           keepClassSpecifications,
                                                      boolean            markClasses,
                                                      boolean            markConditionally,
                                                      boolean            allowShrinking,
                                                      ClassSpecification condition)
            throws ParseException, IOException
    {
        ...
        // Remove the first keep configuration that matches
        KeepClassSpecification spec = parseKeepClassSpecificationArguments(markClasses,
                markConditionally,
                allowShrinking,
                condition);
        boolean removed = keepClassSpecifications.remove(spec);

        if (!removed) {
            throw new ParseException("No keep class specification found to remove (" +
                    "before " + reader.locationDescription() + ")." +
                    "Please ensure that there is a corresponding -keep specification " +
                    "that is exactly the same as the -dontkeep (see also the " +
                    "default proguard-android.txt)");
        }

        // remove all occurencies
        while (removed) {
            removed = keepClassSpecifications.remove(spec);
        }
